...
fun main() {
    ...
    val start = Date().time
    println(redSize)
    println(((BigInteger("1").shl(redSize)).minus(BigInteger("1"))).toString(2))
    println(blacks.size)
    println((BigInteger("1").shl(blacks.size)).toString(2))

    for (bit in (1 shl redSize) - 1 until (1 shl blacks.size)) {
        ...
        val reds = strBit.mapIndexedNotNull { index, c -> if (c == '1') blacks[index] else null }
        val blues = strBit.mapIndexedNotNull { index, c -> if (c == '0') blacks[index] else null }
        println("${reds} | ${blues}")
//        strBit = strBit.padStart(blacks.size, '0')
//
